...
def normalize_id(value: str) -> str:
    """Normalize ID for comparison (Aadhaar digits-only, DL/PAN alphanumeric)."""
...
def ids_equivalent(declared: str, extracted: str) -> bool:
    """True when declared id_number matches document number (DL-aware)."""
...
def check_id_mismatch(
    ...
    is_dl = any(
        doc.get("doc_type") == "driving_licence"
        for doc in document_verdict.get("per_document", [])
    )
    doc_label = "DL No" if is_dl else "document ID"
